...
public class ServiceConsumer {
    ...
    public <T extends Remote> T lookup() {
        ...
        if (size > 0) {
            String url;
            System.out.println("远程服务列表： "+urlList.toString());
            if (size == 1) {
    ...
    public LinkedList<DataNode> allDataNode(){
        System.out.println("远程服务列表： "+urlList.toString());
        LinkedList<DataNode> res= new LinkedList<>();
        int size = urlList.size();
        if(size > 0){
            for (int i = 0;i<urlList.size();i++){
                String url = urlList.get(i);
                DataNode dataNode = lookupService(url);
                res.add(dataNode);
            }
        }
        return res;
            ...
            zk = new ZooKeeper(Constant.ZK_CONNECTION_STRING, Constant.ZK_SESSION_TIMEOUT, new Watcher() {
                ...
                public void process(WatchedEvent event) {
                    if (event.getState() == Event.KeeperState.SyncConnected) {
                        System.out.println("serviceConsumer连接创建成功");
    ...
    private void watchNode(final ZooKeeper zk) {
        try {
            ...
            for (String node : nodeList) {
                ...
                String rmiurl = new String(data);
                //System.out.println(rmiurl);
                if(rmiurl.contains("DataNodeImpl")){
                    try {
                        Master master= (Master) Naming.lookup("rmi://localhost:3099/Master");

                        String dataNodeType = master.getNodesType(rmiurl);
                        System.out.println("节点身份为："+dataNodeType);
                        //System.out.println(dataNodeType.substring(0,6));
